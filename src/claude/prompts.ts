...
export function systemPrompt(country: string): string {
  if (country === "India") {
    return [
      ...
      "client looks like an HNI; (3) INDIA TAX — equity LTCG (₹1.25L/yr exemption, 12.5%",
      "beyond) vs STCG (20%); debt funds taxed at slab with no indexation (post-Apr-2023);",
      "ELSS 80C and lock-ins; NPS 80CCD(1B); SGB interest taxable but capital gains tax-free",
      "at maturity; insurance/ULIP taxation and whether traditional policies are worth",
      "continuing; tax-gain harvesting against the ₹1.25L equity exemption (use the brief's",
      "`gains` block, but note its realBasisPct — estimated bases are NOT real costs);",
      "(4) LIQUIDITY — what",
